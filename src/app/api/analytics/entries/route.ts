...
import { getGateActivity } from "@/server/modules/analytics/analytics.service";
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const gateActivity = await getGateActivity();
  return NextResponse.json({ success: true, gateActivity });
});
